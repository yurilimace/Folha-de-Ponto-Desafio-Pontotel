    ...
    <div>
      <template v-if="search == '' ">
        <table class="table table-bordered  align-self-center">
          <thead>
            <tr>
              <th  class="bg-success table-dark align-middle" v-for="(titulo,index) in colunas" :key='titulo.id'> 
                {{titulo}} 
                <button class="btn btn-success " disabled v-if="index == 1"> 
                  <span  aria-hidden="true" v-if='button'  @click="Asc" > <font-awesome-icon icon="arrow-down" /></span> <span v-else  @click="Dsc"> <font-awesome-icon icon="arrow-up" /> </span>  
                </button>
                <button class="btn btn-success " disabled v-if="index == 0"> 
                  <span  aria-hidden="true" v-if='daybutton'  @click="AscDia"  > <font-awesome-icon icon="arrow-down" /></span> <span v-else   @click="DscDia" > <font-awesome-icon icon="arrow-up" /> </span>  
                </button>  
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="empregado in emp" :key='empregado.id'>
            ...
            <td  class="align-middle" v-for="hora in empregado.pontos" :key='hora.id' > {{Hourformat(hora.time)}}  </td>
          </tr>
        </tbody>
      </table>
    </template>
...
export default {
  ...
  data(){
    ...
    search:'',
    emp:[], 
    colunas:['Dia','Funcionário','Entrada','Inicio da Pausa','Retorno da Pausa','Saida']
    }
  },

  
  mounted(){
    axios.get('https://s3-sa-east-1.amazonaws.com/lar21css/desafio_frontend.json')
    .then(response =>{
  ...
  computed:{
      Filtro(){
          if(!isNaN(this.search) || this.search.includes('/')){
            const x = this.emp.filter((emp)=> this.Dataformat(emp.dia).toLowerCase().includes(this.search.toLowerCase()))
            return x
          }
          else{
            const x = this.emp.filter((emp)=> emp.emp.name.toLowerCase().includes(this.search.toLowerCase()))
            return x
  ...
  methods:{ 
    ...
    listFormat(lista,lista2){
      let newList =[]
      const complete = [{"time":"sem registro de ponto","kind":"entrada"},
      ...
      for(let i=0; i<lista.length;i++){
        for(let j=0;j<lista2.length;j++){
          if(lista[i].id == lista2[j].employee){
            newList.push({emp:lista[i],dia:lista2[j].day,pontos:lista2[j].pontos})
